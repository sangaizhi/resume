...
var resumeUrl = {
    ...
    getSingleParamValue: function (url, paramName) {
        url = decodeURI(url);
        var params = resumeURL.getURLParams(url);
    ...
    removeParam: function (url, paramName) {
        var oldParams = resumeURL.getURLParams(url);
    ...
    addParam: function (url, paramName, paramValue) {
        ...
        if (url.indexOf("?") != -1) {
            ...
            if (!resumeURL.containsParam(url, paramName)) {
                // url本身没有这个参数
                return url + '&' + paramName + "=" + paramValue;
            } else {
                // url 本身有这个参数
...
var resumeDate = {
    ...
    // 格式化时间
    format: function (val) {
        if (!resumeString.isNotBlank(val)) {
            return '';
        }
     ...
     */
    age: function (date) {
        if (!resumeString.isNotBlank(date)) {
            return '';
        }
        var now = new Date();
    ...
    dateDiff: function (date1, date2) {
        date1 = resumeDate.formatWithStyle(new Date(date1), 'yyyy-MM-dd');
        date2 = resumeDate.formatWithStyle(new Date(date2), 'yyyy-MM-dd');
...
/**
 * 正则相关
 * @type {{checkEmail: resumeRegex.checkEmail, checkMobile: resumeRegex.checkMobile}}
 */
var resumeRegex = {
    /**
     * 邮箱正则
...
/**
 * 数字相关
 * @type {{cancelScienceNum: resumeNumber.cancelScienceNum}}
 */
var resumeNumber = {
